...
def test_buy_only_status_is_not_parked_when_buy_offers_are_live():
    ...
    with patch.object(api_server.cfg, "LIQUIDITY_MODE", "buy_only"), \
...
def test_sell_only_status_is_not_parked_when_cat_tier_spares_exist():
    ...
    with patch.object(api_server.cfg, "LIQUIDITY_MODE", "sell_only"), \
...
def test_status_endpoint_feeds_enriched_coin_tracking_into_liquidity_block():
    source = (Path(__file__).resolve().parent.parent / "src" / "catalyst" / "blueprints" / "bot.py").read_text(
        encoding="utf-8"
    )
