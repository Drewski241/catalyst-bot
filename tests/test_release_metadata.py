...
def test_sync_release_metadata_preserves_four_part_versions(tmp_path):
    version_dir = tmp_path / "src" / "catalyst"
    version_dir.mkdir(parents=True)
    ...
    sync_release_metadata(tmp_path, "v1.2.38.3")

    text = version_file.read_text(encoding="utf-8")
    assert '__version__ = "1.2.38.3"' in text
    assert "def get_version" in text
    version_info = (tmp_path / "version_info.txt").read_text(encoding="utf-8")
    assert "filevers=(1, 2, 38, 3)" in version_info
